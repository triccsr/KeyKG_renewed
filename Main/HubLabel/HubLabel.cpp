...
void HubLabel::gen_hub_label_file(const WeightedGraph &ww, int pivotNum) {
  std::cerr<<"Start generating hub label file"<<std::endl;
  ...
  std::vector<VType> pred(static_cast<size_t>(ww.vertex_count(),-1));
  ...
  for(auto & s : ss){
    ...
    std::fprintf(stderr,"%d: vertex %d dijkstra finished, updates HL of %zu vertices\n",vIndex,v,needUpdateLabel.size());
    ++vIndex;
  }
  std::cerr<<"Finish generating hub label file"<<std::endl;
}
