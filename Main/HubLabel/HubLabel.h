 ...
 */
  explicit HubLabel(const char *filePath);
  ~HubLabel();
  static void gen_hub_label_file(const WeightedGraph &ww, int pivotNum, const char *dstFilePath);
  size_t hub_label_size() const;
  double get_dist(VType u, VType v);
  std::vector<VType> get_shortest_path(VType u, VType v);
};
